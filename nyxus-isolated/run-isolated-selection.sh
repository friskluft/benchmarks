...
cp $OUTDIR/timing.txt ./last_timing.txt
...
#=== Leave an experiment mark
echo "--- experiment of $(date) ---" >> $OUTDIR/timing.txt

#=== Extract features
for i in "${arr[@]}"
do
    start=$(date +%s)
    $NYXUSDIR/nyxus --exclusivetiming=false --useGpu=true --verbose=3 --features=mock/"$i" --intDir=$DATADIR/int --segDir=$DATADIR/seg --outDir=$OUTDIR --filePattern=.* --csvFile=singlecsv --loaderThreads=1 --reduceThreads=8
    end=$(date +%s)

    echo "dataset: $DATADIR --- mockname: $i --- time: $(($end-$start)) seconds" >> $OUTDIR/timing.txt
done
